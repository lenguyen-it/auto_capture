...
def start_schedule_loop(hwnd, title, start_time_str, interval_val, unit, base_folder):
    try:
        ...
        print(f"Phát chụp định kỳ tiếp theo sẽ vào lúc: {target_time.strftime('%H:%M:%S')}")
        ...
        while is_running:
            ...
            if current_now >= target_time:
                ...
                target_time += time_delta
                print(f"Lịch chụp tiếp theo đặt vào lúc: {target_time.strftime('%H:%M:%S')}")
            time.sleep(0.5)
...
lbl1 = tk.Label(root, text="Chọn cửa sổ muốn chụp (Chỉ ứng dụng đang mở công khai):", font=("Arial", 10, "bold"))
...
lbl_folder_title= tk.Label(root, text="Chọn folder muốn lưu ảnh chụp:", font=("Arial", 10, "bold"))
...
btn_folder = tk.Button(frame_folder, text="Chọn thư mục", command=select_folder_save_capture, font=("Arial", 9))
btn_folder.pack(side="right")

#Thời gian bắt đầu
lbl2 = tk.Label(root, text="Thời gian bắt đầu lịch trình (Định dạng Giờ:Phút:Giây):", font=("Arial", 10, "bold"))
...
lbl3 = tk.Label(root, text="Khoảng cách giữa các lần chụp định kỳ:", font=("Arial", 10, "bold"))
